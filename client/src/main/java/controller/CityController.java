...
public class CityController extends GameController {
    ...
    public String nextTurn() {
        for (City city : currentPlayer.getCities()) {
            if (city.getConstructingUnit() == null && city.getConstructingBuilding() == null)
                return "you have to choose a production for city : " + city.getId();
        }
        for (City city : currentPlayer.getCities()) {
    ...
    public void createUnit(String name, City city) {
        if (name.equals("Settler"))
            currentPlayer.addUnit(new SettlerUnit(city.getCapital().getX(), city.getCapital().getY()));
        else if (name.equals("Worker"))
            currentPlayer.addUnit(new WorkerUnit(city.getCapital().getX(), city.getCapital().getY()));
        else {
            for (Unit unit : UnitsDatabase.getUnits()) {
                if (unit.getName().equals(name)) {
